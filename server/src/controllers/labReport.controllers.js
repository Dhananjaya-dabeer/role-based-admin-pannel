...
export const createReport = async (req, res, next) => {
  try {
    ...
    if (roles.includes(role)) {
      ...
      return next(
        errorHandler(403, "You are not authorized to do this action")
      );
    }
  } catch (error) {
    next(error);
  ...
  try {
    ...
    if (roles.includes(role)) {
      ...
      if (
        ...
        return next(errorHandler(400, "All fields are rerquired"));
      }
      ...
      if (!updatedReport)
        return next(errorHandler(404, "report doesn't exist with the id"));
      return res.status(201).json({
        success: true,
...
export const getReport = async (req, res, next) => {
  try {
    ...
    if (getRoles.includes(role)) {
      ...
      return next(
        errorHandler(403, "You are not authorized to view this content")
      );
    }
  } catch (error) {
    next(error);
...
export const getSingleReport = async (req, res, next) => {
  try {
    ...
    if (getRoles.includes(role)) {
      ...
      if (!report)
        return next(errorHandler(400, "report not found with id kind"));
      return res.status(200).json({
        success: true,
        statusCode: 200,
        data: report,
