  ...
  const handleFormSubmit = async (e) => {
    ...
    for (let key in formData) {
      if (formData[key].trim() === "") {
        return toast.warn(
          `${key.charAt(0).toUpperCase() + key.slice(1)} can't be empty!`
        );
      }
    }
    try {
      ...
      const respone = await fetch(
        `${import.meta.env.VITE_BACKEND_BASE_URI}/api/create/employee`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="w-full h-full">
      <form onSubmit={handleFormSubmit}>
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500   w-full p-2.5     ">
            <input
              ...
              id="password"
              className="outline-none border-none bg-inherit w-full"
              placeholder="•••••••••"
              onChange={handleChagne}
            />
        ...
        <button
          type="submit"
          className="text-white bg-[#E6A4B4] p-2 rounded-lg hover:bg-[#D28A9E] hover:shadow-lg hover:text-black hover:scale-105 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium  text-sm w-full sm:w-auto px-5 py-2.5 text-center "
        >
          Create
        </button>
      </form>
