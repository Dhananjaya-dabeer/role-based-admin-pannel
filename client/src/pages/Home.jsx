    ...
    (async () => {
      try {
        const labResponse = await fetch(
          `${import.meta.env.VITE_BACKEND_BASE_URI}/api/lab/report/home`,
          {
            method: "GET",
            headers: {
        ...
        const doctorResponse = await fetch(
          `${import.meta.env.VITE_BACKEND_BASE_URI}/api/doctor/report/home`,
          {
            method: "GET",
            headers: {
              "Content-Type": "Application/json",
        ...
        try {
          const employeeResponse = await fetch(
            `${import.meta.env.VITE_BACKEND_BASE_URI}/api/get/list`,
            {
              method: "GET",
              headers: {
                "Content-Type": "Application/json",
          ...
          if (employeestatus) {
            ...
            const doctors = copyOfResult.filter(
            ...
            const moderator = copyOfResult.filter(
              (item) => item.role === "moderator"
            );
            const lab_tech = copyOfResult.filter(
              (item) => item.role === "lab_tech"
            );
